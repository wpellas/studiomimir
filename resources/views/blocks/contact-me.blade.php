<div class="w-full h-full relative flex flex-wrap justify-center my-4 lg:my-8 {{ $block->classes }} {{is_admin() ? "select-none" : ""}}">
  <div class="w-lg xl:w-xl px-4 lg:px-0 h-full z-10 flex flex-wrap justify-center py-10">
    <h1 class="w-full text-center uppercase font-primary text-primary text-xl lg:text-4xl py-6">{{esc_html(strip_tags($title_field))}}</h1>
    <form class="relative w-full lg:w-[800px] h-full flex flex-wrap gap-4 font-primary" action="POST">
      ...
      <div class="w-full h-full flex justify-end">
          <button class="bg-primary py-2 lg:py-4 px-4 lg:px-6 uppercase text-secondary text-base lg:text-xl hover:animate-pulse" type="submit">{{__("skicka")}}</button>
      </div>
    </form>
  </div>
</div>