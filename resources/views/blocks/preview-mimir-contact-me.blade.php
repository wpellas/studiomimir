<div class="px-4 lg:px-0 w-full h-full relative flex flex-wrap justify-center {{ $block->classes }} {{is_admin() ? "pointer-events-none" : ""}}">
  <div class="w-lg h-full z-10 flex flex-wrap justify-center py-8">
    <h1 class="w-full text-center uppercase font-secondary text-primary text-2xl lg:text-4xl py-4">{{esc_html(strip_tags($title_field))}}</h1>
    ...
    <form class="relative w-full lg:w-[800px] h-full flex flex-wrap gap-4 font-primary" action="POST">
      ...
      <div class="w-full h-full flex justify-end">
          <button class="bg-primary py-2 lg:py-4 px-4 lg:px-6 uppercase text-secondary font-secondary text-base lg:text-xl hover:animate-pulse" type="submit">{{__("skicka", 'mimir')}}</button>
      </div>
    </form>
    @endfield
  </div>
